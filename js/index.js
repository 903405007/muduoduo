(function($){
    ...
    $('.menu-dropdown')
        .on('dropdown-show',function(e){
            dropdown.loadOnce($(this),dropdown.builMenuItem);
        })
        .dropdown({
            css3:true,
            js:false
    ...
    dropdown.builMenuItem=function ($elem,data){
        var html='';
...
// todays-slider
    ...
    floor._getImgFormt=function(floorIndex){
        if(floorIndex<3){
            return "jpg";
        }else{
            return "png";
    ...
    floor.buildFloorBody=function (floorData){
        ...
        for(var i=0;i<floorData.items.length;i++){
            ...
            for(var j=0;j<floorData.items[i].length;j++){
                ...
                html+='<p class="floor-item-pic"><a href="###" target="_blank"><img class="floor-img" src="img/floor/loading.gif" data-src="img/floor/'+floorData.num+'/'+(i+1)+'/'+(j+1)+'.'+floor._getImgFormt(floorData.num)+'" alt="" /></a></p>';
    ...
    floor.timeToShow=function (){
        floor.$floor.each(function(index,elem){
            if(lazyLoad.isVisible(floor.floorData[index].offsetTop,floor.floorData[index].height)){
                // console.log('the '+(index+1)+' floor 可见');
    ...
    browser.$doc.on('floor-itemsLoaded',function(){
    ...
    floor.setElevator=function(){
        ...
        if(num===-1){//hide
            // console.log('elevator hide');
            floor.$elevator.fadeOut();
        }else {//show
            // console.log('elevator show');
