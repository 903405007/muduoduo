(function($){
    ...
    Slider.prototype._init=function(){
        ...
        if(this.options.animation==='slide'){
            ...
            this.trasitionClass=this.$items.eq(0).hasClass('transition')?'transition':'';
    ...
    Slider.prototype._slide=function(index,direction){
        ...
        // 设置制定滑入幻灯片的初始位置
        this.$items.eq(index).removeClass(this.trasitionClass).css('left',-1*direction*this.itemWidth);


        // 当前的滑出，指定的滑入
        setTimeout(function(){
            self.$items.eq(self.curIndex).move('x',direction*self.itemWidth);
            self.$items.eq(index).addClass(self.trasitionClass).move('x',0);
            self.curIndex=index;
        },100);
        ...
        slider:function(option){
            return this.each(function(){
                var $this=$(this),
                    ...
                    options=$.extend({},Slider.DEFAULTS,$this.data(),typeof option==='object'&& option);//$this.data()获取data-的对象，active的数值
                if(!slider){
